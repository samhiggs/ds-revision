...
ame = fetch_openml(name="house_prices", as_frame=True)
df = pd.DataFrame(ame.data, columns=ame.feature_names)
df.subset(columns=[""])
...
logging.info("1. Setting index and renaming median_house_value")

# Set index
# df.set_index("id", inplace=True)

# Set Response
df.rename(columns={"median_house_value": "dependent"}, inplace=True)
...
cols_to_cat = ["housing_median_age", "households", "total_rooms", "total_bedrooms"]
